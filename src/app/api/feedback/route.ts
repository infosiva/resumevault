...
export async function POST(req: NextRequest) {
  ...
  const entry = {id:crypto.randomUUID(),site:site??'ResumeVault',type:type??'General',rating,message:message.trim(),email:email?.trim()||null,page:page??'/',createdAt:new Date().toISOString()}
